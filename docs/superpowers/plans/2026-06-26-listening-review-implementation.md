...
- Any standalone script (e.g. `scripts/migrate.ts`) that runs outside Next.js's own dev/build/start process and needs `POSTGRES_URL`/`ANTHROPIC_API_KEY`/`APP_PASSWORD` before it imports `@/lib/db` for any other reason should still load them explicitly with `import { config } from 'dotenv'; config({ path: '.env.local' });` — never the bare `import 'dotenv/config'`, which only loads a file literally named `.env` and silently leaves every var undefined against the `.env.local` that `vercel env pull` actually writes (confirmed by hitting this in Task 1). As of Task 6/9, `src/lib/db.ts` itself also calls this on import, so any file that imports `sql`/`createClient` from `'@/lib/db'` gets env vars for free and does not need its own dotenv call — this covers essentially every test file from Task 9 onward.
