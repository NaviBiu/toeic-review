    ...
    await withTestClient(async (client) => {
      const { rows: [ids] } = await client.query(
        `WITH parent AS (
           ...
           SELECT '14111111-1111-4111-8111-111111111111', session_id, question_id, 'B', false, '2026-08-20T08:00:00Z'
           FROM session_item
           UNION ALL
           SELECT '24222222-2222-4222-8222-222222222222', session_id, question_id, 'A', true, '2026-08-20T08:00:00Z'
           FROM session_item
         )
         SELECT
