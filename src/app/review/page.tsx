                ...
                <section className="work-review-item mt-6 border border-slate-300 bg-white">
                  ...
                  <div className="p-6">
                    ...
                    {phase === 'guessing' ? (
                      ...
                      <div className="mt-6 border-t border-slate-200 pt-5"><p className="font-mono text-sm text-slate-600">Decision recorded for this reference. Continue when ready to review the next item.</p><div className="mt-5 flex items-center justify-between gap-3"><div className="flex items-center gap-3"><span className={'rounded-full px-3 py-1 font-mono text-xs ' + (guess === 'remember' ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-800')}>{guess === 'remember' ? workReviewCopy.confirmed : workReviewCopy.followUp}</span><button onClick={revoke} title={workReviewCopy.revert} aria-label={workReviewCopy.revert} className="rounded-full p-1.5 text-slate-500 hover:bg-slate-100 hover:text-slate-900">↩</button></div><button onClick={next} className="border border-slate-700 bg-slate-700 px-4 py-2 font-mono text-sm text-white hover:bg-slate-800">{workReviewCopy.continue}</button></div></div>
                    )}
                  </div>
                </section>
              )}
