...
export type ReadingCategoryMutation = {
  type: 'rename' | 'move';
  sourceId: number;
...
export default function ReadingCategoryManager({
  ...
  onMutation?: (mutation: ReadingCategoryMutation) => void;
}) {
  ...
  function beginAction(action: string) {
    if (busyActionsRef.current.has(action)) return false;
  ...
  async function createCategory() {
    ...
    const action = 'create';
    if (!name || !beginAction(action)) return;
    setError('');
    try {
  ...
  async function renameCategory(category: ReadingNoteCategory) {
    ...
    const action = `rename:${category.id}`;
    if (!name || !beginAction(action)) return;
    setError('');
    try {
  ...
  async function reorder(category: ReadingNoteCategory, offset: -1 | 1) {
    ...
    const action = `reorder:${category.id}`;
    if (index < 0 || target < 0 || target >= active.length || !beginAction(action)) return;
  ...
  async function deleteCategory() {
    ...
    const action = `delete:${deleteTarget.id}`;
    ...
    try {
      ...
      const targetCategory = destination === 'uncategorized'
        ? active.find((category) => category.isDefault) ?? null
        : typeof destination === 'number'
          ? active.find((category) => category.id === destination) ?? null
          : null;
      onChanged(active
        .filter((category) => category.id !== deleteTarget.id)
        .map((category) => targetCategory && category.id === targetCategory.id
          ? { ...category, noteCount: category.noteCount + deleteTarget.noteCount }
          : category));
      onMutation?.({
        type: 'move',
        sourceId: deleteTarget.id,
        targetId: targetCategory?.id ?? null,
        targetName: targetCategory?.name ?? null,
      });
  ...
  async function mergeCategory() {
    ...
    const action = `merge:${mergeSource.id}`;
    if (!beginAction(action)) return;
    setError('');
    try {
      const target = active.find((category) => category.id === Number(mergeTargetId));
      if (!target) throw new Error('请选择目标分类');
      ...
      onChanged(active
        ...
        .map((category) => category.id === target.id
          ? { ...category, noteCount: category.noteCount + mergeSource.noteCount }
          : category));
      onMutation?.({
        type: 'move',
    ...
    <section aria-label="阅读笔记分类管理">
      <div className="border-b border-stone-200 pb-5">
        ...
        <div className="mt-2 flex gap-2">
          ...
          />
          <button type="button" onClick={() => void createCategory()} disabled={isBusy('create') || !newName.trim()} className="rounded-md bg-stone-900 px-4 py-2 text-sm font-medium text-white hover:bg-stone-800 disabled:bg-stone-300">
            新增分类
          </button>
        </div>
      </div>
          ...
          <li key={category.id} className="flex flex-wrap items-center gap-3 py-3">
            <div className="min-w-0 flex-1">
              {editingId === category.id ? (
                <div className="flex gap-2">
                  ...
                  <button type="button" onClick={() => void renameCategory(category)} disabled={isBusy(`rename:${category.id}`)} className="text-sm font-medium text-stone-900">保存</button>
                  <button type="button" onClick={() => setEditingId(null)} disabled={isBusy(`rename:${category.id}`)} className="text-sm text-stone-500">取消</button>
                </div>
              ) : (
                <>
            ...
            {editingId !== category.id ? (
              <div className="flex items-center gap-1">
                <button type="button" aria-label="上移分类" title="上移" onClick={() => void reorder(category, -1)} disabled={isBusy(`reorder:${category.id}`) || index === 0} className="h-8 w-8 rounded-md text-stone-500 hover:bg-stone-100 disabled:text-stone-200">↑</button>
                <button type="button" aria-label="下移分类" title="下移" onClick={() => void reorder(category, 1)} disabled={isBusy(`reorder:${category.id}`) || index === active.length - 1} className="h-8 w-8 rounded-md text-stone-500 hover:bg-stone-100 disabled:text-stone-200">↓</button>
                {!category.isDefault ? (
                  <>
                    <button type="button" aria-label="重命名分类" title="重命名" onClick={() => { setEditingId(category.id); setEditingName(category.name); }} disabled={isBusy(`rename:${category.id}`)} className="h-8 w-8 rounded-md text-stone-500 hover:bg-stone-100">✎</button>
                    <button type="button" aria-label="合并分类" title="合并" onClick={() => { setMergeSource(category); setMergeTargetId(''); }} disabled={isBusy(`merge:${category.id}`)} className="h-8 w-8 rounded-md text-stone-500 hover:bg-stone-100">⇥</button>
                    <button type="button" aria-label="删除分类" title="删除" onClick={() => { setDeleteTarget(category); setDeleteDestination('uncategorized'); }} disabled={isBusy(`delete:${category.id}`)} className="h-8 w-8 rounded-md text-stone-400 hover:bg-red-50 hover:text-red-700">🗑️</button>
                  </>
                ) : null}
              </div>
            ) : null}
      ...
      {mergeSource ? (
        <div role="dialog" aria-label="合并分类" className="mt-4 border border-stone-300 bg-stone-50 p-4">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            ...
            <button type="button" onClick={() => void mergeCategory()} disabled={isBusy(`merge:${mergeSource.id}`) || !mergeTargetId} className="rounded-md bg-stone-900 px-4 py-2 text-sm font-medium text-white disabled:bg-stone-300">确认合并</button>
            <button type="button" onClick={() => setMergeSource(null)} disabled={isBusy(`merge:${mergeSource.id}`)} className="px-3 py-2 text-sm text-stone-600">取消</button>
          </div>
        </div>
      ) : null}
      ...
      {deleteTarget ? (
        <div role="alertdialog" aria-label="删除分类" className="mt-4 border-l-2 border-red-700 bg-red-50 p-4">
          ...
          <div className="mt-3 flex gap-3">
            <button type="button" onClick={() => void deleteCategory()} disabled={isBusy(`delete:${deleteTarget.id}`)} className="rounded-md bg-red-700 px-4 py-2 text-sm font-medium text-white disabled:bg-red-300">删除分类</button>
            <button type="button" onClick={() => setDeleteTarget(null)} disabled={isBusy(`delete:${deleteTarget.id}`)} className="px-3 py-2 text-sm text-stone-600">取消</button>
          </div>
        </div>
      ) : null}
    </section>
